...
var WebsocketClient = function(productID, websocketURI, opts={}) {
  var self = this;
  self.productID = productID || 'BTC-USD';
  self.websocketURI = websocketURI || 'wss://ws-feed.gdax.com';
  self.reconnectOnClose = !!opts.reconnectOnClose;
  self.shouldTerminate = !self.reconnectOnClose;
  ...
  prototype.onOpen = function() {
    ...
    self.pinger = setInterval(function(){
      try {
        ...
        console.error("Ping failed. Maybe the socket isn't open? Reason:", err);
      }
    }, 30000);

  };

  prototype._onClose = function() {
    console.log(`Connection closed at ${new Date()}`);
    var self = this;
    clearInterval(self.pinger);
    self.socket = null;
    if (self.reconnectOnClose && !self.shouldTerminate) {
