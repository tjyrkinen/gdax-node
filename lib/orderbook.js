...
_.assign(Orderbook.prototype, new function() {
  ...
  // For a given trade size, what is the highest price on individual quotes matched
  prototype.getPriceAtVolumeDepth = function(side, amount) {
    var iterator = null;
    var step = null;

    if (side == 'ask') {
      iterator = this._asks.iterator();
      step = iterator.next.bind(iterator);
    }
    else if (side == 'bid') {
      iterator = this._bids.iterator();
      step = iterator.prev.bind(iterator);
    }
    else {
      throw new Error('No trade side specified');
    }

    var cumAmount = 0;
    var currentPrice = 0;
    var item;
    while (item = step() !== null && cumAmount < amount) {
      const data = iterator.data();

      currentPrice = Number(data.price.valueOf());
      data.orders.forEach(function(order) {
        cumAmount += Number(order.size.valueOf());
      });
    }

    return [currentPrice, cumAmount];
  };
